      ...
      <section className="mx-auto grid w-full max-w-3xl gap-4 px-4 pt-8">
        <div className="rounded-2xl bg-white p-6 shadow-panel">
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-600">
            ...
            <p>Verified: <strong>{user.email_verified_at ? "Yes" : "No"}</strong></p>
            <p>Tenant: <strong>{user.tenant_id}</strong></p>
          </div>
          {!user.email_verified_at ? (
            <form action={requestVerificationAction} className="mt-4">
              <button className="rounded-lg border border-ocean px-3 py-2 text-sm font-semibold text-ocean">
