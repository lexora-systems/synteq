...
const allowedScenarios = new Set(["webhook-failure", "retry-storm", "latency-spike", "duplicate-webhook"]);
...
async function postSimulation(accessToken: string, scenario: string, body: unknown) {
  return fetch(`${apiBaseUrl}/v1/simulate/${scenario}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
export async function POST(request: Request, context: { params: Promise<{ scenario: string }> }) {
  const { scenario } = await context.params;
  if (!allowedScenarios.has(scenario)) {
    return NextResponse.json({ error: "Unsupported scenario" }, { status: 404 });
  }
  ...
  let response = accessToken ? await postSimulation(accessToken, scenario, body) : null;
  if (!response || response.status === 401) {
    ...
    response = await postSimulation(refreshed.accessToken, scenario, body);
  }
