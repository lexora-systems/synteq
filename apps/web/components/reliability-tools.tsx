  ...
  async function runSimulation(scenario: "webhook-failure" | "retry-storm" | "latency-spike" | "duplicate-webhook") {
    ...
    try {
      ...
      setSimulationMessage(
        `${payload.result.injected_events} synthetic events injected for ${payload.result.scenario}. These are manual validation signals, not scheduled monitoring.`
      );
    } catch (error) {
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <section className="rounded-2xl bg-white p-6 shadow-panel">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Manual validation</span>
          <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">Not scheduled monitoring</span>
        </div>
        <p className="mt-1 text-sm text-slate-600">
          Run user-triggered simulations to validate detection behavior. These are not scheduled synthetic monitors and may create
          operational signals or incidents.
        </p>
        ...
        <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
          Simulations use the real monitoring path; incidents may appear after simulation depending on the scenario and current alert
          configuration. Scheduled synthetic monitoring is not enabled yet.
        </div>
